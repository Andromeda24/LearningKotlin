...
class ImageGenViewModel(
    ...
    fun generateImage() {
        Log.i("ImageGenViewModel", "generateImage() called")
        viewModelScope.launch {
            ...
            val result = withContext(Dispatchers.IO) {
                Log.i("ImageGenViewModel", "generateImage() withContext called")
                imageRepository.generateImage(prompt = currentState.value.prompt)

            }
            result.onSuccess {images->
                Log.i("ImageGenViewModel", "generateImage() onSuccess ")
                _imageGenUiState.update {
                    it.copy(
                        myImages = images,
                        isLoading = false,
                ...
                Log.i("ImageGenViewModel", "generateImage() onFailure ${throwable.message}")
                _imageGenUiState.update {
                    it.copy(
                        isLoading = false,
